    ...
    ** It might not work if the input 'x' is not a square.
    ...
    def __init__(self):
        ...
        self.down_1 = nn.Sequential(
            conv_block(3, 16),
            conv_block(16, 32, stride=2, padding=1))  # [b, 32, 112, 112]
    ...
    def forward(self, x):
        down1 = self.down_1(x)
        out = F.max_pool2d(down1, kernel_size=2, stride=2)  # [b, 32, 56, 56]

        down2 = self.down_2(out)  # [b, 128, 56, 56]
        out = F.max_pool2d(down2, kernel_size=2, stride=2)  # [b, 128, 28, 28]

        out = self.middle(out)  # [b, 128, 28, 28]

        out = F.upsample(out, scale_factor=2)  # [b, 128, 56, 56]
        out = torch.cat([down2, out], 1)  # [b, 128*2, 56, 56]
        out = self.up_2(out)  # [b, 32, 56, 56]

        out = F.upsample(out, scale_factor=2)  # [b, 32, 112, 112]
        out = torch.cat([down1, out], 1)  # [b, 64, 112, 112]
        out = self.up_1(out)  # [b, 32, 112, 112]

        out = F.upsample(out, scale_factor=2)  # [b, 32, 224, 224]
        out = self.output(out)
        return out
