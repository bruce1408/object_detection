...
def fit_ont_epoch(net, yolo_losses, epoch, epoch_size, epoch_size_val, gen, genval, Epoch, cuda):
    ...
    torch.save(model.state_dict(), 'checkpoints_1/Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth' % (
        (epoch + 1), total_loss / (epoch_size + 1), val_loss / (epoch_size_val + 1)))  # 模型保存
...
if __name__ == "__main__":
    ...
    # annotation_path = '0728_train.txt'
    annotation_path = "1016_train.txt"
    # -------------------------------#
    #   获得先验框和类
    # -------------------------------#
    ...
    #   权值文件的下载请看README
    # -------------------------------------------#
    model_path = "./checkpoints/Epoch49-Total_Loss2.5157-Val_Loss8.5791.pth"  # 加载预训练模型进行训练
    # 加快模型训练的效率
