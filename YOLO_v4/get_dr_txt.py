...
class mAP_Yolo(YOLO):
    def __init__(self, single_model=True):
        super(mAP_Yolo, self).__init__()
        self.single_model = single_model
        if single_model:
            self.path = "./input/detection-result_single_model/"
        else:
            self.path = "./input/detection-result_multi_model/"
        print(self.path)
    ...
    def detect_image(self, image_id, image):
        self.confidence = 0.3  # draw box over than 0.05 will display

        f = open(self.path + image_id + ".txt", "w")
