...
class CustomData(Dataset):
    def __init__(self, image_path="data/BagImages", mode="train"):
        ...
        self.image_list = [i.split("/")[-1] for i in glob(os.path.join(self.image_path, "*.jpg"))]
        self.mode = mode

        if mode in ("train", "val"):
            self.mask_path = self.image_path + "Masks"

        self.transform_x = T.Compose([T.Resize((256, 256)),
                                      T.ToTensor(),
    ...
    def __getitem__(self, index):
        if self.mode in ("train", "val"):
            ...
            X = Image.open(os.path.join(self.image_path, self.image_list[index]))
...
if __name__ == "__main__":
    data = CustomDataset("data/testImages", mode="val")
    print(data[0])